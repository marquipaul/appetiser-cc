...
export default {
  ...
  methods: {
    save () {
      this.loading = true
      if (this.form.id) {
        axios.put(`http://127.0.0.1:8000/api/events/${this.form.id}`, this.form)
        .then(res => {
          this.getEvent()
          this.loading = false
        })
        ...
        axios.post(`http://127.0.0.1:8000/api/events`, this.form)
          .then(res => {
            this.getEvent()
            this.loading = false
          })
    ...
    enumerateDaysBetweenDates(startDate, endDate) {
      let currDate = moment(startDate).startOf('day').clone();
